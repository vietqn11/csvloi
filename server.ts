...
*Yêu cầu về văn phong:* Diễn đạt ngắn gọn, khúc chiết, mang đậm tính kỹ thuật hệ thống thực tế và tuyệt đối không sáo rỗng. Tránh lạm dụng các từ ngữ mô tả chung chung không thể hành động. Sử dụng thuật ngữ kỹ thuật chuyên ngành chuẩn mực.`;
      ...
      for (const modelName of modelsToTry) {
        try {
          console.log(`[Gemini API] Attempting log analysis with model: ${modelName}`);
          ...
          if (response && response.text) {
            text = response.text;
            console.log(`[Gemini API] Successfully generated analysis report using model: ${modelName}`);
            break;
          }
        } catch (err: any) {
          lastApiError = err;
          console.warn(`[Gemini API] Model ${modelName} returned error: ${err.message || err}`);
          // If the model is busy, we continue immediately to the next fallback model
        }
      }
      ...
      if (!text) {
        throw new Error(
          `Không thể hoàn thành phân tích nhật ký bằng mô hình AI nào (tất cả các dịch vụ đều đang bận hoặc quá tải). Lỗi chi tiết: ${
            lastApiError?.message || lastApiError || "Unknown API Error"
          }`
        );
      }
